...
# ======================================================
# Temporal Anomaly Detection v2 — 改进版
# 改进：数据标准化 + 加深模型 + 验证集阈值优化
# ======================================================
...
def find_best_threshold(val_errors, val_labels):
    """在验证集上寻找最佳阈值（最大化F1）"""
    best_f1 = 0
    best_threshold = 0
    for percentile in np.arange(50, 100, 0.5):
        threshold = np.percentile(val_errors, percentile)
        pred = (val_errors > threshold).astype(int)
        TP = np.sum((val_labels == 1) & (pred == 1))
        FP = np.sum((val_labels == 0) & (pred == 1))
        FN = np.sum((val_labels == 1) & (pred == 0))
...
def main():
    print("=" * 70)
    print("Temporal Anomaly Detection v2 (Improved)")
    print("=" * 70)
    ...
    # 划分：正常段(0-700)，异常段(700-800)，测试段包含正常和异常
    normal_data = trajectory_scaled[:700, :]
    test_data = trajectory_scaled[500:, :]  # 后500步，含正常和异常
    ...
    # 用验证集选择阈值
    print("\n在验证集上选择最佳阈值...")
    threshold, best_val_f1 = find_best_threshold(val_errors, val_labels)
    print(f"最佳阈值: {threshold:.6f}")
    print(f"验证集F1: {best_val_f1:.3f}")
    ...
    plt.savefig(os.path.join(images_dir, 'temporal_anomaly_detection_v2.png'), dpi=150, bbox_inches='tight')
    print(f"\n图表已保存到 {os.path.join(images_dir, 'temporal_anomaly_detection_v2.png')}")
    plt.show()
