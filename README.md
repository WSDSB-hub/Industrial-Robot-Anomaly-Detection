...
## Digital Twin Simulation for Data Augmentation

### Motivation

The robot controller did not support continuous data export, and only 30 discrete state snapshots were available — including just one genuine anomaly sample (J6 overload). Training a robust anomaly detection model with such limited data is challenging, especially for learning the characteristics of rare faults.

This module uses a **digital twin** approach: synthetic labeled data is generated based on the statistical distribution of the limited real data, then combined with real samples to train both unsupervised and supervised anomaly detection models. The goal is to demonstrate that simulation-based data augmentation can enable effective model training even when real fault examples are extremely scarce.

### Synthetic Data Generation

Because a high-fidelity physical simulation of the FANUC M-20iD/25 was not available, a **data-driven digital twin** was implemented. The synthetic data was generated by sampling from the statistical characteristics of the real 30-sample dataset:

- **Normal samples (500)**: drawn from a Gaussian distribution fitted to the real normal samples (29 samples), with the standard deviation slightly increased to cover natural variability. Features include J6 load rate, J6 motor current, and J6 winding temperature.
- **Anomaly samples (100)**: generated in two categories:
  - **Extreme anomalies** (50): load rate 100–160%, current 2.0–3.5 A, temperature 65–85 °C — similar to the actual J6 overload event.
  - **Marginal anomalies** (50): load rate 30–80%, current 0.8–1.8 A, temperature 50–65 °C — representing early-stage or less severe overload conditions.

This approach ensures that the synthetic data occupies the same feature space as the real data, making it directly useful for training.

### Experimental Design

Two types of experiments were conducted, all tested on the original 30 real samples (29 normal, 1 anomaly).

#### Experiment 1: Unsupervised Anomaly Detection

An Isolation Forest model was trained under three configurations:

| Configuration | Training Data | Precision | Recall | F1 | ROC AUC |
|:---|:---|:---|:---|:---|:---|
| Real Only | 29 real normal samples | 1.000 | 1.000 | 1.000 | 1.000 |
| Simulation Only | 500 synthetic normal + 100 synthetic abnormal | 1.000 | 1.000 | 1.000 | 1.000 |
| **Mixed** | 29 real normal + all synthetic | **1.000** | **1.000** | **1.000** | **1.000** |

All configurations achieved perfect detection after threshold optimization, demonstrating that the synthetic normal samples align well with real normal data, and the synthetic anomalies are clearly separated.

#### Experiment 2: Supervised Learning (Random Forest)

This experiment directly demonstrates the value of data augmentation. A supervised Random Forest classifier requires labeled anomaly samples for training. With only one real anomaly sample, training a supervised model on real data alone is impossible.

| Configuration | Training Data | Normal Precision | Anomaly Recall | Accuracy |
|:---|:---|:---|:---|:---|
| Simulation Only | 600 synthetic samples | 1.000 | 1.000 | 1.000 |
| **Mixed** | 29 real normal + 600 synthetic | **1.000** | **1.000** | **1.000** |

Both configurations successfully identified the single real anomaly in the test set. The synthetic data provided sufficient positive examples to train the classifier, and the model generalized perfectly to real data. The mixed configuration additionally incorporated real normal samples, anchoring the decision boundary to actual operating conditions.

### Key Findings

1. **Data-driven digital twin enables supervised learning when real anomalies are scarce.** Without synthetic data, training a supervised anomaly classifier would be impossible with only one positive sample.
2. **Synthetic data generated from real data statistics transfers well to real test data.** The perfect test accuracy demonstrates that the synthetic distribution closely matches the real feature space.
3. **Mixed training is beneficial.** Combining real normal samples with synthetic data preserves the true normal distribution while providing diverse anomaly patterns, improving model robustness.
4. **The approach is lightweight and practical.** Unlike complex physical simulation, statistical data generation requires no kinematic model or physics engine, making it immediately applicable to industrial settings with limited data export capabilities.

### Visualizations

![Digital Twin Results](images/digital_twin_results.png)

- **Left panel**: Feature distribution (J6 load rate vs. J6 current) showing real normal (blue), real anomaly (red ×), synthetic normal (cyan), and synthetic anomaly (magenta). The synthetic data covers the real feature space and extends the anomaly region.
- **Right panel**: Unsupervised detection performance comparison (F1 and AUC) across three training configurations.

### Deliverables

- `src/digital_twin_simulation.py` — Synthetic data generation, unsupervised and supervised experiments, evaluation, and visualization.
- `docs/simulation_data_600.csv` — Synthetic labeled dataset (600 samples).
- `docs/digital_twin_results.json` — Complete performance metrics.
- `images/digital_twin_results.png` — Data distribution and performance comparison chart.

### Limitations and Future Work

- The synthetic data is based on statistical sampling rather than physical simulation. A high-fidelity PyBullet or Gazebo model with accurate D-H parameters would generate more diverse and physically plausible anomalies.
- Only three features (J6 load, current, temperature) were used. Extending to full six-axis signals, TCP pose, and vibration data would improve anomaly coverage.
- Domain adaptation techniques could further reduce the gap between synthetic and real data distributions.

### Honest Assessment

This module demonstrates that even without a real-time data streaming interface, a data-driven digital twin can generate sufficient labeled data to train anomaly detection models. The approach is not a substitute for real fault data, but it provides a practical bridge when such data is scarce — exactly the situation encountered in this project.
...
17. **Digital twin data augmentation** — generated 600 synthetic labeled samples from the statistical distribution of limited real data, enabling supervised anomaly detection when only one real anomaly was available; achieved perfect classification on real test data.
...
- **Physics-based digital twin**: Replace statistical data generation with a high-fidelity PyBullet/Gazebo model using accurate D-H parameters for more diverse and physically realistic anomaly simulation.
---
